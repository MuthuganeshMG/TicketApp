...
function Ticket() {
    ...
    const fetchTickets = async () => {
        try {
            const response = await fetch('http://localhost:3597/api/tickets')
            const data = await response.json();
            setTickets(data);
        } catch (error) {
            // setTickets('data not found');
        // console.error(
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await
                fetch('http://localhost:3597/api/create', {
                    method: 'POST',
                    headers: {
    ...
    const handleDelete = async (ticketId) => {
        try {
            await
                fetch(`http://localhost:3597/api/delete/${ticketId}`, {
                    method: 'DELETE',
                });
    ...
    const handlePriorityChange = async (ticketId, newPriority) => {
        try {
            const response = await
                fetch(`http://localhost:3597/api/update/${ticketId}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
